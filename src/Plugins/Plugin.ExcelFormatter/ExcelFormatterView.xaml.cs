...
{
    private ExcelFormatterViewModel? _viewModel;
    ...
    public ExcelFormatterView(Plugin.Abstractions.IPluginContext? context = null)
    {
        ...
        AttachViewModel(DataContext as ExcelFormatterViewModel);
    }

    private void OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (e.OldValue is ExcelFormatterViewModel oldVm)
        {
        ...
        AttachViewModel(e.NewValue as ExcelFormatterViewModel);
    }

    private void AttachViewModel(ExcelFormatterViewModel? vm)
    {
        _viewModel = vm;
    ...
    {
        if (PendingFilesDataGrid.Columns.Count == 0) return;
