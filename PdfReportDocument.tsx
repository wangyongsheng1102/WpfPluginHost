...
import {
  MAX_PROCESS_KEYS_PER_PAGE,
  getProcessKeysFromTable,
  getProcessTablePageCount,
} from './pdf/processTableConstants';
...
export const PdfReportDocument: React.FC<PdfReportDocumentProps> = ({ context }) => {
  ...
  const processKeys = getProcessKeysFromTable(context.process_table ?? []);
  const processPageCount = getProcessTablePageCount(processKeys);
  const totalPages = 4 + partsPageCount + processPageCount;
  ...
  return (
    <Document>
      ...
      {Array.from({ length: processPageCount }, (_, i) => {
        const start = i * MAX_PROCESS_KEYS_PER_PAGE;
        const slice =
          processKeys.length === 0
            ? []
            : processKeys.slice(start, start + MAX_PROCESS_KEYS_PER_PAGE);
        return (
          <Page key={`process-${i}`} size="A4" orientation="landscape" style={styles.page}>
            <ProcessTablePage
              context={context}
              processKeys={slice}
              pageNum={5 + partsPageCount + i}
              totalPages={totalPages}
              showFootnote={i === processPageCount - 1}
            />
          </Page>
        );
      })}
